...
#   puts product.ean
  ...
  begin
    img = wait.until { driver.find_element(css: '.photoswipe__image.lazyautosizes.lazyloaded').attribute('data-photoswipe-src') }
    if img
      product.link = img
    end
  rescue Selenium::WebDriver::Error::TimeoutError
...
#   puts product.unit
  ...
  begin
    ...
    product.box = "Carton biberons x12"
  rescue Selenium::WebDriver::Error::TimeoutError
    product.box = nil
  end
  puts product.box
...
#   puts product.ttc_box_price
  ...
  product.category = Category.find_by(name: "Sauces")
...
# brand = "Alphaform"
  ...
  driver.navigate.to "https://www.sauces-mixo.com/collections/les-bibs"
  driver.manage.timeouts.implicit_wait = 10
  main_window = driver.window_handle

  marque = Brand.last
  # marque = Brand.create(name: brand)
  links = wait.until { driver.find_elements(css: '.grid__item.grid-product.small--one-half.medium-up--one-quarter.grid-product__hover-details.aos-init.aos-animate a') }
  links.each do |a|
